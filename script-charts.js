...
function renderChartList() {
  ...
  if (!category || !category.shows || category.shows.length === 0) {
    container.innerHTML = '<p class="loading">No chart data yet for this category — run the update workflow to populate it.</p>';
    return;
  }
...
function renderTrendingVideos(trending) {
  ...
  if (videos.length === 0) {
    container.innerHTML = '<p class="loading">No trending video data yet — run the YouTube update workflow first.</p>';
    return;
  }
...
function renderTopChannels(trending) {
  ...
  if (channels.length === 0) {
    container.innerHTML = '<p class="loading">No channel data yet — run the YouTube update workflow first.</p>';
    return;
  }
  ...
  try {
    ...
    if (chartsData.categories && chartsData.categories.length > 0) {
      ...
      document.getElementById('chart-list').innerHTML =
        '<p class="loading">No chart data yet — run the "Update Podcast Charts" workflow once.</p>';
    }
    ...
    console.error(err);
    document.getElementById('chart-list').innerHTML = '<p class="loading">Could not load chart data.</p>';
  }
}
